...
\begin{enumerate}
    ...
    Technically, an agent is a sandboxed computation (e.g., a Docker container or VM) that exposes three items:
    \begin{itemize}
        \item A definition of the state transition logic.

        \item A definition of the execution proof verification logic.

        \item An entry point that accepts the inputs, executes the computation, and returns the outputs along with a proof of correct execution.
    \end{itemize}
    The users/clients use the first definition to determine whether the agent will perform the actions they want, then call the entry point with their inpus, and finally verify the correctness of the computed outputs by feeding the defintion of the computation, the inputs, the outputs, and the proof to the verification logic. Note that the verification step is executed by the callers themselves, so they can rely on this being done correctly.
